...
function SWEP:PrimaryAttack()
    ...
    local trace = util.TraceLine({
        ...
        indoors = true
        sshoot = self:GetValue("Sound_Shoot_Indoors")
    end
    ...
    if self:GetValue("Silencer") then
        ...
        sshoot = self:GetValue("Sound_Shoot_Silenced_Indoors")
        end
    end
        ...
        if shootent or !hitscan or fixed_spread then
            ...
            for i = 1, num do
                ...
                elseif hitscan then
                    ...
                    TacRP:ShootPhysBullet(self, self:GetMuzzleOrigin(), new_dir:Forward() * self:GetValue("MuzzleVelocity") *3)
                end
            end
        else
            local new_dir = Angle(dir)
...
function SWEP:ShootRocket(dir)
    ...
    if phys:IsValid() and self:GetValue("ShootEntForce") > 0 then
        phys:SetVelocityInstantaneous(dir:Forward() * self:GetValue("ShootEntForce") *5)
    end
end
