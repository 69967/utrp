...
function ENT:Initialize()
    ...
    self.NextUse = 0
	self:SetColor( Color( 77,153,0, 255 ) )
...
function ENT:ApplyAmmo(ply)
    ...
    if currentAmmo >= maxAmmo then
        ply:ChatPrint("9x19mm Slurpcorp full [" .. maxAmmo .. "]")
        return
    elseif currentAmmo + ammoToGive > maxAmmo then
        ammoToGive = maxAmmo - currentAmmo
        ply:ChatPrint("All 9x19mm Slurpcorp rounds didn't fit, got (" .. ammoToGive .. ")")
    end
...
if CLIENT then
    ...
    function ENT:Draw()
        ...
        local resolution = 9
        local ammoType = "9x19mm Slurpcorp"
