...
function ENT:Initialize()
    ...
    self.NextUse = 0
	self:SetColor( Color( 153,170,255, 255 ) )
...
function ENT:ApplyAmmo(ply)
    ...
    if currentAmmo >= maxAmmo then
        ply:ChatPrint("9x19mm Match full [" .. maxAmmo .. "]")
        return
    elseif currentAmmo + ammoToGive > maxAmmo then
        ammoToGive = maxAmmo - currentAmmo
        ply:ChatPrint("All 9x19mm Match rounds didn't fit, got (" .. ammoToGive .. ")")
    end
